...
public class BMallet {
	private static String dirLDA = BGlobalVar.Results;
...
//				dirLDA + "en.txt"), "UTF-8", false, false, false) );
		...
		Reader fileReader = new InputStreamReader(new FileInputStream(
				new File(dirLDA + "LDADocs.txt")), "UTF-8");
...
////////////////////////////////////////////////////////////////////		
	    ...
	    model.write(new File(dirLDA + "LDAmodel.bin"));
	    instances.save(new File(dirLDA + "LDAInstances.bin"));
	}
	...
	private static void malletTests2() throws Exception {
		...
		InstanceList instances = InstanceList.load(
				new File(dirLDA + "LDAInstances.bin"));
		Alphabet dataAlphabet = instances.getDataAlphabet();
	...
	private static void CreateFile() throws Exception {
		PrintStream ps = new PrintStream(dirLDA + "LDADocs.txt");
	...
	private static void initModels() throws Exception {
		MModel = ParallelTopicModel.read(new File(BGlobalVar.LDAModel));
		MInstances = InstanceList.load(new File(BGlobalVar.LDAInstances));
	...
	public static void GetTopics(int numTopics) throws Exception {
		...
		PrintStream ps = new PrintStream(dirLDA + "LDATopics.txt");
	...
	public static double[] getDistribution(String text) throws Exception {
		...
		MInferencer.setRandomSeed(234344);
	...
	public static void turnoffLoggers() {
		MalletLogger.getLogger("").setLevel(Level.OFF);
	}
